...
@modulus.sym.main(config_path="conf3", config_name="config")
def run(cfg: ModulusConfig) -> None:
    ...
    panel_origin = (-.01, -.055) # (m, m) left bottom corner
    panel_dim = (0.02, 0.11) # (m, m)
    gauge_dim = (.0125, .06) # (m, m)
    gauge_origin = (-.00625, -.03) # (m, m)
    circle_radius = 0.013 # (m)
    circle1_origin = (-.00625 - circle_radius, -.03) # (m, m)
    circle2_origin = (-circle1_origin[0], -circle1_origin[1]) # (m, m)
    circle3_origin = (circle1_origin[0], -circle1_origin[1]) 
    circle4_origin = (-circle1_origin[0], circle1_origin[1])
    gauge = Rectangle(gauge_origin, (gauge_dim[0] + gauge_origin[0], gauge_origin[1] + gauge_dim[1]))
    panel = Rectangle(panel_origin, (panel_origin[0] + panel_dim[0], panel_origin[1] + panel_dim[1]))
    rec = Rectangle((panel_origin[0], gauge_origin[1]), (panel_origin[0] + panel_dim[0], gauge_origin[1] + gauge_dim[1]))
    c1 = Circle(circle1_origin, circle_radius)
    c2 = Circle(circle2_origin, circle_radius)
    c3 = Circle(circle3_origin, circle_radius)
    c4 = Circle(circle4_origin, circle_radius)
    thckness = 3e-3 #m

    # final geometry after remove those circle form the rectangular panel
    geo = panel - rec - c1 - c2 - c3 - c4 + gauge
    ...
    panel_left_2 = PointwiseBoundaryConstraint(
            ...
            outvar={"traction_x": 0.0, "traction_y": 0.0},
            batch_size = cfg.batch_size.panel_left,
            criteria = Eq(x, gauge_origin[0]) & (y >= gauge_origin[1]) & (y <= gauge_origin[1] + gauge_dim[1]), # equations to be need to defined here of constarints
            parameterization={t: bounds_t, x:gauge_origin[0], y:(gauge_origin[1], gauge_origin[1] + gauge_dim[1])}, # t is a sympy symbol!!
            batch_per_epoch=500,
        )
    ...
    panel_right_2 = PointwiseBoundaryConstraint(
            ...
            outvar={"traction_x": 0.0, "traction_y": 0.0},
            batch_size = cfg.batch_size.panel_right,
            criteria = Eq(x, gauge_origin[0] + gauge_dim[0]) & (y >= gauge_origin[1]) & (y <= gauge_origin[1] + gauge_dim[1]), # defining the equations (constraints)
            parameterization={t: bounds_t, x:(gauge_origin[0] + gauge_dim[0]), y:(gauge_origin[1], gauge_origin[1] + gauge_dim[1])}, # parametric terms
            batch_per_epoch=500,
        )
